            ...
            commonService.todayTotal().then(function(res){
                ...
                commonService.dailyonlinemax({dailymax:dailymax}).then(function(res){
                    ...
                    commonService.countuser().then(function(res) {
                        ...
                        //定义时间区间数组
                        // function getHourArea(yesterdayStarTime){
                        //     debugger;
                        //     var hourAreas = [];
                        //     for (var i=0;i<=23;i++){
                        //         var ha ={};
                        //         ha['begin']=yesterdayStarTime+i*60*60*1000;
                        //         ha['end']=yesterdayStarTime+(i+1)*60*60*1000;
                        //         hourAreas.push(ha);
                        //     }
                        //     return hourAreas;
                        // }
                        ...
                        commonService.WithinOneday().then(function(res) {
                            ...
                            if(res.online.length!=0) {
                                ...
                                console.log($scope.onlineHour[0].data);
                                // var proHourData=[];
                                // var yesterday =Date.parse(changeDate(new Date()));
                                // var hourAreas =getHourArea(yesterday);//获取24个小时的时间段
                                // console.log(hourAreas);
                                for(var i=0;i<$scope.onlineHour.length;i++){
                                    // var proHourDataObj={};
                                    // var hourData=[];
                                    // var productId = $scope.onlineHour[i].productId;
                                    // var minuteData=$scope.onlineHour[i].data;
                                    // for(var j=0;j<=minuteData.length;j++){
                                    //     for(var n=0;n<hourAreas.length;n++){
                                    //         // if(hourAreas[n].begin<minuteData[j].timestamp)
                                    //     }
                                    // }
