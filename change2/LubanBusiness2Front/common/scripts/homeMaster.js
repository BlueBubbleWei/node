...
app.controller('businessController', function ($scope,$compile,$location) {
    ...
    $scope.$watch('leftState',function (countOrder,newValue,oldValue) {
        if(countOrder>2){
            countOrder=0;
        }
        ...
        switch (TState){
            case 0:
                switch (LState){
                    ...
                    case 3:$location.path("/userAnalysis");break;
                    case 4:$location.path("/functionAnalysis"); break;
                    case 5:$location.path("/questionAnalysis"); break;
                    default:break;
                }
    ...
    function setNavigationLeft(state) {
        ...
        var setHtml=function () {
            ...
            for(var i=1;i<paramList.length;i++){
                tempHtml+="<ul class='naviLeftItem' ng-bind='orderstate='"+i+ "><li class='maincontent'>"+"<div class='fakeBotBor'><img src="+(paramList[i].iconImage?paramList[i].iconImage:paramList[0].commonImg)+" />"+paramList[i].title+"</div></li>";
