function enterpriseFilterController() {
    ...
    var queryLBEnterpriseListSuccess=function (data) {
        ...
        scope.EntListCount=dataObj.result.count;  // 共多少条数据
        if(!fromPaging) {
        ...
        var setHtml=function (itemFH,itemH) {
            ...
            for(var i=0;i<enterpriseList.length;i++){
                tempHtml+='<div class="EnterpriseListItems" style="height: '+itemH.toString()+'px;">' +
    ...
    var deleteEnterprise=function (enterpriseId) {
        ...
        new LBPopupInput({
            data:{
                ...
                content:[
                    { id:0,type:'tips',descript:'',data:"被筛减的企业将不参与所有指标的统计！",leftWidth:'11%'}
    ...
    // 初始化页面
    var doRootSetController=function ($scope,$compile,isShow) {
        //isShow = false;
        scope=$scope;
        compile=$compile;
