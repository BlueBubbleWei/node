...
function setContentMarginL() {
    var contentMarginLeft=($('body').width()-naviLeftMargin)/2-$('#contentPage').width()/2-60;
...
app.controller('businessController', function ($scope,$compile,$location) {
    ...
    function setNavigationLeft() {
        $scope.paramList = [
                        {
                            pMenu:{name:"企业客户分析",icon:"",uiSref:""},
                            subMenu:[{name:"企业概况",icon:"",uiSref:"enterpriseGeneral"},{name:"企业列表",icon:"",uiSref:"enterpriseList"}]
                        },
                        {
                            pMenu:{name:"系统使用分析",icon:"",uiSref:""},
                            subMenu:[{name:"系统概况",icon:"",uiSref:"systemView"},{name:"用户分析",icon:"",uiSref:"userAnalysis"},{name:"功能分析",icon:"",uiSref:"funAnalysis"},{name:"问题分析",icon:"",uiSref:"questionAnalysis"}]
                        },
                        {
                            pMenu:{name:"管理",icon:"",uiSref:""},
                            subMenu:[{name:"权限设置",icon:"",uiSref:"rootSetting"},{name:"成员管理",icon:"",uiSref:"memberManage"},{name:"企业筛选",icon:"",uiSref:"enterpriseFilter"}]
                        },
                    ];
        var setHtml=function () {
            var tempHtml='';

            tempHtml+= "<div class='naviLeftItem' ng-repeat='item in paramList track by $index'><div><span>{{item.pMenu.name}}</span></div><ul><li ng-repeat='sub in item.subMenu track by $index'><span ui-sref={{sub.uiSref}}>{{sub.name}}</span></li></ul></div>"+"<div class='naviLeftItemGap'></div>"
...
app.directive('clickLeft',function ($compile, $parse) {
    return{
        ...
        link: function(scope, element, attr) {
            $(".count-number").click(function($event){
                $(this).addClass('switch-left-down').removeClass('switch-left-up');
                // $($event.target).addClass("switch-left-down");
...
app.directive('spreadButton',function ($compile, $parse) {
    return{
        restrict:'EA',
        link: function(scope, element, attr) {
