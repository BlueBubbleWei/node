...
function MemManController() {
    ...
    //查询角色种类
    var  getQueryLbRoleList="http://"+dbPort+"/rest/lbrole/roleList";
    //请求用户列表
    ...
    var setPagination=function (start,size,count,DOM) {
        ...
        var refreshPageStart=function (start) {
            pageStart=start;
            postQueryLBUserParam.page = pageStart;
            scope.$apply(function () {
                scope.pageStart=start;
            });
            searchMemberList(-2);
   ...
   //成功查询所有角色数据
    var queryLBUserListSuccess=function (data) {
        ...
        if(!fromPaging) {
            ...
            setPagination(pageStart, postQueryLBUserParam.lineage, dataObj.result.sum, 'paging');
        }
        fromPaging=false;
        var setHtml=function (itemFH,itemH) {
            ...
            var changeInfoStyle=changeStyle+"background-image:url('../common/images/Lock-Icon.png');";
    ...
    var queryRoleList=function () {
        getAjax(getQueryLbRoleList, JSON.stringify({}), queryLBRoleLSuccess, postError('queryLBRoleList'),true);
    };
    // 初始化页面
