        ...
        listenSelect: function() {
            ...
            self.$container.on('click', '[name]', function() {
                ...
                if( $(this).index()==3 && $(this).text()=='自定义'){
                    ...
                    createCaldar();
                    if($('#create-date').length>0){
                        initCaldar('create-date');//初始化生成日历
                    }
                }else{
                    $('#create-date').empty();
                    self.$input.val(self.dataList[self._seletedIndex].desc? self.dataList[self._seletedIndex].desc:$this.text());
...
(function ($) {
    ...
    var isEmpty=function(item){
        if(item==''||typeof(item)=='undefined'||item==null||item=='null')return '--';
        else return item;
    }
    var assemblyAreaInfo=function(province,city,region) {
        // 省-城市(地区)
    ...
    LBPopupInput.prototype={
            ...
            for(var item in self.contentList){
                ...
                switch (self.contentList[item].type){
                    case 'input':
                        ...
                        var inputValue = ((data!=null) && (data != undefined) && (data!='')) ? data:'';
                        var input=$('<input value="'+inputValue+'"/>');
                        input.css({'border':'none','backgroundColor':'transparent','width':'97%','height':'38px','lineHeight':'38px','fontSize':'16px','padding':'0  10px','margin-left':'1%'});
                    ...
                    case 'textArea':
                        ...
                        var data = self.contentList[item].data;
                        var dataValue = ((data!=null) && (data != undefined) && (data!='')) ? data:'';
                        var textArea=$('<textarea>'+dataValue+'</textarea>');
                        ...
                        break;
                    case 'checkbox':
                        if(self.options.data.title.onlyTitle)break;
                        ...
                        if(self.contentList[item].isSelect){
                    ...
                    case 'multiCheckbox':
                        ...
                        break;
                    case 'tips':
                        titleDiv.css({'width':((self.contentList[item].leftWidth!=null) && (self.contentList[item].leftWidth != undefined) && (self.contentList[item].leftWidth!='')) ? self.contentList[item].leftWidth:'20%'});
                        var inputDiv=$('<div></div>');
                        inputDiv.css({'width':((self.contentList[item].rightWidth!=null) && (self.contentList[item].rightWidth != undefined) && (self.contentList[item].rightWidth!='')) ? self.contentList[item].rightWidth:'60%'});
                        inputDiv.css({'height':'40px','float':'left','margin':((perH-40)/2+10).toString()+'px 10px'});
                        inputDiv.appendTo(itemDiv);
                        if(self.contentList[item].icon){
                            var titleIcon=$('<span class="bombBoxIcon bombBoxIcon-'+self.contentList[item].iconIndex+'"></span>');
                            titleIcon.appendTo(inputDiv);
                        }

                        var data = self.contentList[item].data;
                        var dataValue = ((data!=null) || (data != undefined) || (data!='')) ? data:'';
                        var input=$('<span>'+dataValue+'</span>');
                        input.css({'border':'none','backgroundColor':'transparent','width':'100%','height':'40px','lineHeight':'40px','fontSize':'16px','position':'relative','top':'-4px','fontWeight':'bold'});
                        input.attr('name','content-'+self.contentList[item].id.toString());
                        input.appendTo(inputDiv);
                        break;
                    case 'icon':
                        itemDiv.css({'height':'60px','margin-top': '13px'});
                        titleDiv.css({'width':'44%'});
                        var inputDiv=$('<div></div>');
                        inputDiv.css({'height':'60px','width':'60px','float':'left','margin':'-1px','background-color': '#15aa10','border-radius': '30px'});
                        inputDiv.appendTo(itemDiv);
                        var leftDiv=$('<div></div>');
                        leftDiv.css({'position': 'relative','width': '17px','height': '6px','left': '11px','top': '38px','background-color': 'white','transform': 'rotate(45deg)','transform-origin':'right center','border-radius': '3px'});
                        leftDiv.appendTo(inputDiv);
                        var rightDiv=$('<div></div>');
                        rightDiv.css({'position': 'relative','width': '27px','height': '6px','left': '26px','top': '30px','background-color': 'white','transform': 'rotate(-45deg)','transform-origin':'left center','border-radius': '3px'});
                        rightDiv.appendTo(inputDiv);
                        break;
                    case 'messages':
                        titleDiv.css({'display':'none'});
                        var multiDataDiv=$('<div></div>');
                        multiDataDiv.css({'overflow-y': 'auto','height': '160px','width': '100%'});
                        var data = self.contentList[item].data;
                        var leftWidth = ((self.contentList[item].leftWidth!=null) && (self.contentList[item].leftWidth != undefined) && (self.contentList[item].leftWidth!='')) ? self.contentList[item].leftWidth:'20%';
                        var rightWidth = ((self.contentList[item].rightWidth!=null) && (self.contentList[item].rightWidth != undefined) && (self.contentList[item].rightWidth!='')) ? self.contentList[item].rightWidth:'60%';
                        for(var key in data){
                            var leftDiv=$('<div>'+key+'</div>');
                            leftDiv.css({'height':'40px','paddingRight':'3%','fontSize':'16px','lineHeight':perH.toString()+'px','textAlign':'left','float':'left','marginLeft':'10%'});
                            leftDiv.css({'width':leftWidth});
                            leftDiv.appendTo(multiDataDiv);
                            var rightDiv=$('<div></div>');
                            rightDiv.css({'width':rightWidth});
                            rightDiv.css({'height':'40px','float':'left','margin':((perH-40)/2).toString()+'px 10px'});
                            rightDiv.appendTo(multiDataDiv);
                            var spanArea=$('<span>'+data[key]+'</span>');
                            spanArea.css({'border':'none','backgroundColor':'transparent','width':'100%','height':'40px','lineHeight':'40px','fontSize':'16px','padding':'0  10px','color':'#AA1019'});
                            spanArea.attr('name','content-'+self.contentList[item].id.toString());
                            spanArea.appendTo(rightDiv);
                        }
                        multiDataDiv.appendTo(itemDiv);
                        break;
                    case 'enterpriseInfo':
                        itemDiv.css({'width':'90%','height':perH*4+'px','margin':'20px 32px'});
                        titleDiv.css({'display':'none'});
                        var data = self.contentList[item].data;
                        var textOver='text-overflow: ellipsis;white-space: nowrap;overflow: hidden;';
                        var commonStyle='float:left;height:40px;';
                        var titleDiv1=$('<div style="'+commonStyle+'">企业名称:</div>');
                        titleDiv1.css({'width':'15%'});
                        titleDiv1.appendTo(itemDiv);
                        var dataDiv1=$('<div style="'+textOver+commonStyle+'" title="'+isEmpty(data.enterprise)+'">'+isEmpty(data.enterprise)+'</div>');
                        dataDiv1.css({'width':'35%','color':'#808080'});
                        dataDiv1.appendTo(itemDiv);
                        var titleDiv2=$('<div style="'+commonStyle+'">部署类型:</div>');
                        titleDiv2.css({'width':'15%'});
                        titleDiv2.appendTo(itemDiv);
                        var dataDiv2=$('<div style="'+textOver+commonStyle+'" title="'+isEmpty(data.type)+'">'+isEmpty(data.type)+'</div>');
                        dataDiv2.css({'width':'35%','color':'#808080'});
                        dataDiv2.appendTo(itemDiv);
                        var titleDiv3=$('<div style="'+commonStyle+'">服务器版本:</div>');
                        titleDiv3.css({'width':'15%'});
                        titleDiv3.appendTo(itemDiv);
                        var dataDiv3=$('<div style="'+textOver+commonStyle+'" title="'+isEmpty(data.serverVersion)+'">'+isEmpty(data.serverVersion)+'</div>');
                        dataDiv3.css({'width':'35%','color':'#808080'});
                        dataDiv3.appendTo(itemDiv);
                        var titleDiv4=$('<div style="'+commonStyle+'">授权时间:</div>');
                        titleDiv4.css({'width':'15%'});
                        titleDiv4.appendTo(itemDiv);
                        var dataDiv4=$('<div style="'+textOver+commonStyle+'" title="'+isEmpty(data.authorizationTime)+'">'+isEmpty(data.authorizationTime)+'</div>');
                        dataDiv4.css({'width':'35%','color':'#808080'});
                        dataDiv4.appendTo(itemDiv);
                        var titleDiv5=$('<div style="'+commonStyle+'">维护时间:</div>');
                        titleDiv5.css({'width':'15%'});
                        titleDiv5.appendTo(itemDiv);
                        var dataDiv5=$('<div style="'+textOver+commonStyle+'" title="'+isEmpty(data.renewTime)+'">'+isEmpty(data.renewTime)+'</div>');
                        dataDiv5.css({'width':'85%','color':'#808080'});
                        dataDiv5.appendTo(itemDiv);
                        var titleDiv6=$('<div style="'+commonStyle+'">地区:</div>');
                        titleDiv6.css({'width':'15%'});
                        titleDiv6.appendTo(itemDiv);
                        var areaInfo=assemblyAreaInfo(data.province,data.city,data.region);
                        var dataDiv6=$('<div style="'+textOver+commonStyle+'" title="'+areaInfo+'">'+areaInfo+'</div>');
                        dataDiv6.css({'width':'85%','color':'#808080'});
                        dataDiv6.appendTo(itemDiv);
                        var titleDiv7=$('<div style="'+commonStyle+'">详细地址:</div>');
                        titleDiv7.css({'width':'15%'});
                        titleDiv7.appendTo(itemDiv);
                        var dataDiv7=$('<div style="'+textOver+commonStyle+'" title="'+isEmpty(data.address)+'">'+isEmpty(data.address)+'</div>');
                        dataDiv7.css({'width':'85%','color':'#808080'});
                        dataDiv7.appendTo(itemDiv);
                        break;
            ...
            var getThisValue=function (item) {
                ...
                switch (item.type){
                    ...
                    case 'textArea':
                        ...
                        break;
                    case 'checkbox':
                        if($('input[name^="content-'+item.id.toString()+'"]').is(":checked")) {
                            retV=1; // 选中 1
                        } else {
                            retV=0; // 选中 0
                        }
                    ...
                    case 'multiCheckbox':
                        var data = item.data;
                        var name = 'content-'+item.id.toString()+'-';
                        retV=[];
                        var retVIndex=0;
                        for(var i=0;i<data.length;i++){
                            if($('input[name='+name+data[i].id+']').is(":checked")) {
                                retV[retVIndex]=data[i].id;
                                retVIndex++;
                            }
        ...
        addTitle:function () {
            ...
            if(!self.options.data.content)
                self.titleH=self.popH*0.4;
            if(self.options.data.title.onlyTitle)
                self.titleH=self.popH*0.4;
            ...
            if(self.options.data.title.titleIcon){
                var titleIcon=$('<span class="bombBoxIcon bombBoxIcon-'+self.options.data.title.iconIndex+'"></span>');
                var temp = self.options.data.title.marginTop;
                var marginTop =((temp==null)&&(temp==undefined)&&(temp==''))?0:temp;
                titleIcon.css({'marginTop':marginTop+'px'});
                titleIcon.appendTo(self.popTitle);
            }

            var titleInfo=$('<span>'+self.title+'</span>');
            titleInfo.css({'fontSize':fontSize.toString()+'px'});
            titleInfo.appendTo(self.popTitle);
            ...
            if(this.endButList.length>0){
                self.endButList.forEach(function (item , i) {
                    ...
                    switch (item.type){
                        ...
                        case 'func':
                            if(hasCheckbox) {
                                self.endButListJQ[i].css({'position':'relative','left':'180px','top':'-45px'});
                            }
                            self.endButListJQ[i].click(function () {
                        ...
                        case 'checkbox':
                            ...
                            var checkboxDiv=$('<div></div>');
                            checkboxDiv.attr('class','lbCheckBox');
                            checkboxDiv.css({'position':'relative','left':'-57px','top':'20px','float':'left','width':'58%'});
                            var input=$('<input />');
                            input.attr('type','checkbox');
                            input.attr('name','content-'+item.id.toString());
                            input.attr('id','content-'+item.id.toString());
                            input.appendTo(checkboxDiv);
                            var label =$('<label />');
                            label.attr('for','content-'+item.id.toString());
                            label.appendTo(checkboxDiv);
                            var labelInfo =$('<label >'+item.value+'</label>');
                            labelInfo.css({'marginLeft':'10px','position':'relative','top':'-4px'});
                            labelInfo.attr('for','content-'+item.id.toString());
                            labelInfo.appendTo(checkboxDiv);
                            self.endButListJQ[i]=checkboxDiv;
                            break;
                        default:break;
                    }
