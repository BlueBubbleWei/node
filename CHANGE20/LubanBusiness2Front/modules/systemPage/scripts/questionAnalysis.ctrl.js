...
//be浏览器-->productId:12;//iBan-->productId:13;//进度计划->productId:23;// bw（集成应用）-->productId:29;// mc(驾驶舱) productId:11;//移动应用//BV pad-->productId:27;//BV 手机版-->productId:28;
    ...
    var isShowCancelBtn = 1;//是否显示去下筛选条件按钮
    ...
    ajaxParams.queryProblemNub.nubParams = {productId:productId};
    ajaxParams.queryType.typeParams = {productId:productId};
    ajaxParams.queryVersions.versitionParams = {productId:productId};
    ...
    function productTopNav(obj){//产品导航条
        ...
        $('.analysis-Nav-list li').eq(0).children('.line-style').addClass( 'showStyle').show();

    }

    function startStroke(obj){
        obj.on('mouseover mouseout','li',function(){
            if(!$(this).find('.line-style').hasClass('showStyle')){
                if(event.type == "mouseover"){
                    $(this).children('.line-style').show();//单机导航条的显示样式
                }else if(event.type == "mouseout"){
                    $(this).children('.line-style').hide();
                }
            }
        })
        obj.on('click','li',function(){
            currentPage = 0;
            $(this).children('.line-style').addClass('showStyle').show().parent().siblings().children('.line-style').hide().removeClass('showStyle');//单机导航条的显示样式
            ...
            getAjax(ajaxParams.queryType.typeUrl+productId,null,ajaxParams.queryType.typeSuccess,ajaxParams.queryType.typeError,true);
            getAjax(ajaxParams.queryVersions.versitonsUrl+productId,null,ajaxParams.queryVersions.versionsSuccess,ajaxParams.queryVersions.versionsError,true);
        });
    ...
    function echartShape(data){
        var shapeData = [];
        var  colorArr = ['#8B0F3D','#E24856','#FD9029','#63D0DA','#34B1E4','#055381','#6524DD','#8D59ED','#4B0783','#8C9CFF','#FFCF4E','#3DCD9C'];
        for(var i= 0;i<data.length;i++){
            ...
            shapeObj.itemStyle.normal.color = colorArr[i];
...
//getAjax(ajaxParams.downFile.downDownUrl,null,ajaxParams.downFile.downSuccess,ajaxParams.downFile.downError,true);
    ...
    ajaxParams.queryList.listSuccess = function(data){
        ...
        for(var i = 0;i<queryProData.list.length;i++){
            dataList = queryProData.list[i];
                tr +='<tr eleId = '+dataList.id+'>'
                    +'<td>'+dataList.type+'</td>'
                    +'<td class="substr questionList-con" title="'+dataList.content+'" style="max-width:560px;word-break: break-all;word-wrap: break-word">'+dataList.content+'</td>'
        ...
        $('.analysis-table table tr:even').css({'background':'#f5f5f5'});
        $('.analysis-table .content-table'). off ( 'click','tr' );//解除ajax绑定事件绑定失效
    ...
    function templateInputList(data){//生成全部为题类型
        var  inputList = '';
        for(var i = 0;i<data.length;i++){
            if(typeof data[i]==='string' && !isNaN(data.length)){
                inputList += "<p class='quest-type'>&nbsp;&nbsp;"+data[i]+"</p>";
            }
            if(typeof data[i]==='object' && !isNaN(data.length)){
                inputList += "<p class='quest-type' typeId = "+data[i].typeId+">&nbsp;&nbsp;"+data[i].typeName+"</p>";
            }
        }
    ...
    function hideInput(eventElement,hideElement){//离开、单机下拉框调用的元素
        eventElement.mouseenter(function(){
            ...
            $(this).find('.triangle').css({'border-color':'#aa1019'});
        });
        eventElement.click(function(){
            $(this).find(hideElement).slideDown();
            $(this).addClass('ques-isOpen');
            $(this).find('.triangle').css({'transform':'rotate(-224deg)','border-color':'#aa1019','margin-right':'15px'});
        });
        ...
        //    $(this).find(".triangle").css('border-color','');
        //    if( $(this).hasClass('ques-isOpen')){
        //        $(this).find('.triangle').css({'transform':'rotate(-44deg)','border-color':'','margin-right':'10px'});
        //    }
        //});
        $(document).click(function(){
          $('#quesAllType>.anlaysis-inputWap').hide();
        });
    }
    ...
    $scope.filterStatus = function(){
        ...
        //console.info('问题列表展示',versitionText,typeId,deployType,postLChartParam.beginTime,postLChartParam.endTime);
            $('.ques-cancel').css({'display':'inline-block'})
    ...
    function creatPopup(data){//生成弹窗
            ...
            for( var key in isNullData.result){
                ...
                popupMsg = '<div class="ques-analysis-popup"><div class="ques-popup-con">'
                                    +'<div class="ques-title">'
                                      +'<div class="popup-tips">已复制到剪切板</div>'
                                         +'<p class="ques-title-icon">'
                                             +'<span class="bombBoxIcon bombBoxIcon-7"></span>'
                                             +'<span>问题详情</span>'
                                         +'</p>'
                                         +'<p class="ques-close">'
                                    ...
                                    +'<div class="ques-content">'
                                        ...
                                        +'<div class="container question-overflow">'
                                            +'<ul class="gallery">'
                                                +li
                                            +'</ul>'
                                        +'</div>'
        ...
        $('.ques-close').bind('click',function(){
    ...
    $("body").append('<div id="zoom" ><a class="close"><span class="icon-close"></span></a><a href="#previous" class="previous"></a><a href="#next" class="next"></a><div class="question-view"><div class="content loading" style="height:auto"></div></div><div class="page-num"></div></div>');
    var t = $("#zoom").hide(),
        n = $("#zoom .content"),
        r = false,
        i = null,
...
//--------------------------------查询问题类型--------------------------------------------
    ajaxParams.queryType.typeSuccess = function(data){
        data = JSON.parse(data);
        //var ttt = templateInputList(data.result);
        $('.analysis-inputs #quesAllType .anlaysis-inputWap').remove();
...
//------------------------------------版本查询
    ajaxParams.queryVersions.versionsSuccess = function(data){
        ...
        $('#quesAllVersition .anlaysis-inputWap').remove();
