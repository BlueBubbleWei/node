...
app.controller('userAnalysisController', function ($scope,$compile,$location,$timeout,commonService) {
    ...
    $scope.todayList=[];
    $scope.onlineHour=[];
    var result,average,max,date,registerlist,loginlist,flag=1;
    ...
    function initExcel(){
        ...
        for(var i=0;i<$scope.product.length;i++){
            for(var j=0;j<$scope.todayList.length;j++){
                if ($scope.todayList[j].productId == $scope.product[i].productId) {
                    var userloginData=$scope.todayList[j].data;
            ...
            if (!$scope.product[i].todayLogin) {
                $scope.product[i].todayLogin = 0;
            }
            if (!$scope.product[i].yestodayLogin) {
                $scope.product[i].yestodayLogin = 0;
            }
        }
        //日均访问量拼接
        for(var i=0;i<$scope.product.length;i++){
        ...
        //当前在线人数拼接
        for(var i=0;i<$scope.product.length;i++){
            for(var j=0;j<$scope.onlineUser.length;j++){
                if ($scope.onlineUser[j].productId == $scope.product[i].productId) {
                    var dataLens=$scope.onlineUser[j].data.length;
                    for(var k=0; k<dataLens; k++){
                        $scope.product[i].user =$scope.onlineUser[j].data[k].users ;
                    }
                }
            }
                    ...
                    commonService.countuser().then(function(res) {
                        ...
                        if(res.data.length!=0) {
                            ...
                            commonService.producturl().then(function(res) {//获取产品名称
                                ...
                                initExcel();//调用表格渲染
                                initWaveChart();//图表2初始化

                                $scope.onLineChartSeries = [];
                                ...
                                commonService.WithinOneday().then(function(res) {
                                    ...
                                    if(res.online.length!=0) {
                                        ...
                                        for(var i=0;i<$scope.onlineHour.length;i++){
                                            var pname = '';
                                            for(var m=0;m< $scope.psdList.length;m++){
                                                if($scope.onlineHour[i].productId==$scope.psdList[m].productId){
                                                    pname = $scope.psdList[m].productName
                                                }
                                            }
                                            for(var m=0;m< $scope.calList.length;m++){
                                                if($scope.onlineHour[i].productId==$scope.calList[m].productId){
                                                    pname = $scope.calList[m].productName
                                                }
                                            }
                                            for(var m=0;m< $scope.otherList.length;m++){
                                                if($scope.onlineHour[i].productId==$scope.otherList[m].productId){
                                                    pname = $scope.otherList[m].productName
                                                }
                                            }
                                            initLegend(i,pname);
                                            var onlineHourData=[];
                                            for(var j=0;j<$scope.onlineHour[i].data.length;j++){
                                                var thisData = $scope.onlineHour[i].data[j];
                                                onlineHourData.push({
                                                    name: thisData.time,
                                                    value: [ thisData.time,  thisData.users ]
                                                })
                                            }
                                            var serie= {
                                                name:pname,
                                            ...
                                            $scope.onLineChartSeries.push(serie)
                                        }
                                    }
                                ...
                                option = {
                                    ...
                                    series:$scope.onLineChartSeries
                                };
