...
app.controller('userAnalysisController', function ($scope,$compile,$location,$timeout,commonService) {
    ...
    var currentOnlineData=[];//当前在线人数的实时更新
    ...
    function drawOnlineTable(){
        ...
        for(var i=0;i<onlyTableDatas.length;i++){
            if(i==0){
                html+="<tr id='currentOnline'><th class='TabLeft'>当前在线人数</th><th>";
    ...
    function getOnlieUsersDatas(){
        ...
        for(var i=0;i<$scope.groupProduct.length;i++) {
            ...
            currentOnlineData.push(count_users);
            currentOnlineData.push(currentOnlineTable);
        }
        console.log(JSON.stringify(currentOnlineData)+'currentOnlineData')
        console.log('1111111')
    }
    ...
    * */
    ...
    function refreshTableOnlineUser() {
        var html_table="<th class='TabLeft'>当前在线人数</th>";
        $("#currentOnline").empty();
        for(var i=0;i < currentOnlineData.length; i++){
            if(currentOnlineData[i] instanceof Array){
                for(var j=0;j<currentOnlineData[i].length;j++){
                    html_table+="<th>"+currentOnlineData[i][j]+"</th>"
                }
            }else{
                html_table+="<th>"+currentOnlineData[i]+"</th>"
            }
        }
        html_table +="</th>";
        console.log(html_table+'html_table')
        $("#currentOnline").append(html_table);
