...
//getAjax(ajaxParams.downFile.downDownUrl,null,ajaxParams.downFile.downSuccess,ajaxParams.downFile.downError,true);
    ...
    ajaxParams.queryList.listSuccess = function(data){
        ...
        //console.info('查询问题列表',queryProData);
        var tr = "";
        var  dataList = [];
        for(var i = 0;i<data.length;i++){
            dataList = queryProData.list[i];
            //dataList = queryProData.list[i];
            console.info(queryProData.list[0])
    ...
    //
    //}
    getAjax(ajaxParams.queryList.queryListUrl,null,ajaxParams.queryList.listSuccess,ajaxParams.queryList.listError,true);
//queryProblemDeatail
