...
var sureShow=function (list) {
    isShow = list[0].value==0?true:false;
}
...
app.config(function ($stateProvider,$urlRouterProvider,$controllerProvider){
    ...
    $stateProvider
        ...
        .state("systemView", {
            ...
            resolve: {
                deps: app.resolveScriptDeps([
                    '/LubanBusiness2Front/common/scripts/system-user.js',
                ])
            },
    ...
    function setNavigationLeft(state) {
        commonService.getMenu('lubanb').then(function(data){
            ...
            var setHtml=function () {
                ...
                tempHtml+= "<div class='naviLeftItem' ng-repeat='item in paramList track by $index'><div ng-click='toggle($event)'><img ng-src={{item.pMenu.img}} /><span>{{item.pMenu.menu}}</span></div><ul style='display: none'><li ng-repeat='sub in item.subMenu track by $index'><span ui-sref={{sub.uri}}>{{sub.menu}}</span></li></ul></div>"
                return tempHtml
            };
            var html=setHtml();
            var template=angular.element(html);
